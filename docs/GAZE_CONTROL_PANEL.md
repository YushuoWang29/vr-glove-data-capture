...
该功能不会自动修改 `Assets/NoitomHi5`、`Assets/Hi5_Interaction_SDK` 或原厂 `.unity` 场景。运行时安装器在每次进入 Play Mode 时建立一个全新的有效实例，仅在检测到原厂 `MenuStateMachine`、`VREyeRaycaster`、`SelectionRadial` 和 `VRInteractiveItem` 后创建面板，因此没有加载原厂 UI 的其他场景不会出现空面板。缺失依赖持续 2 秒后会在 Console 输出具体等待项，而不是静默失败。
...
功能按钮**不会在校准前显示**。运行时控制器通过反射直接订阅厂商权威回调 `HI5.HI5_Calibration.OnCalibrationComplete`，只有收到 `PPose` 才解锁；`HI5_Manager_Thread.IsCalibrationComplete` 与 `CalibrationStateMachine.Finish` 仅作为回退信号。完成回调会先锁存到下一帧，再等待厂商自己的状态处理结束后切换 UI，因此不会漏掉短暂的 `Finish` 状态。
...
- `GazeFunctionPanelController` 在每次 Play Mode 建立独立运行时实例，找到原厂主状态节点，并在完整校准后隐藏原主页面子项、显示项目功能面板。
- 功能面板复制原厂 `Main` 的空间变换，但作为它的同级对象存在。原厂左手物理按钮把菜单切到 `Exit` 时，已解锁的功能面板不会再随 `Main` 一起被隐藏。
- P-pose 完成检测通过反射订阅厂商静态回调，保持仓库对本地 Hi5 SDK 的软依赖；销毁控制器时会可靠退订。
...
测试实际加载本地 `TableScene_Vive`，验证本次 Play 的有效安装器和回调订阅均已建立、原厂校准对象仍存在、`Interaction` 位于 `Reconnect` 右侧且碰撞体不重叠、`PPose` 权威回调能在 manager 标志为 false 时解锁、菜单随后切到 `Exit` 也不会隐藏面板、六个功能按钮均接入原厂视线交互、完成注视能够切换透视请求，以及重新校准能够恢复经过适配的原厂主界面。视觉断言还会检查背景亮度、灰黑文字、圆角网格和文字不越界；透视断言会检查 `VerticalStereo` 的左右眼区域映射及各眼内部 180° 旋转不越区。
...
确认已经完成完整 **P-pose**，而不是只完成 V-pose 或 B-pose。Console 中应依次出现 `[GazeControlPanel] Runtime installer created`、`Bound to HI5_Calibration.OnCalibrationComplete`、`Installed on the vendor main menu` 和 `Function controls unlocked via HI5_Calibration.OnCalibrationComplete(PPose)`。若只有第一条，后续节流警告会明确指出缺少 `MenuStateMachine`、`VREyeRaycaster`、`SelectionRadial` 或 `Main` 根对象；若连第一条也没有，当前运行的脚本还不是本版本。不要把 manager 标志或瞬时 `Finish` 当作唯一诊断依据。
